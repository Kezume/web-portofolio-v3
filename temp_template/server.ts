...
import { readDB, writeDB, logVisitor, getAnalytics } from "./src/server/db";
...
// Visitor logging middleware (Exclude assets, dev endpoints and API calls)
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  if (!isStatic && !isVite && req.method === "GET") {
    const ip = req.headers["x-forwarded-for"] as string || req.socket.remoteAddress || "127.0.0.1";
    // Standard clean page matching
    let page = req.path;
    if (page === "") page = "/";
    logVisitor(ip, page, req.headers.referer || "", req.headers["user-agent"] || "");
  }
  next();
});
...
app.get("/api/portfolio", (req: Request, res: Response) => {
  try {
    const db = readDB();
    const { adminPasswordHash, visitorMetrics, ...publicData } = db;
    res.json(publicData);
  } catch (error) {
...
// 5. Experiences CRUD
app.post("/api/admin/experiences", requireAdmin, (req: Request, res: Response) => {
  try {
    const db = readDB();
    const newExp = {
      id: "exp_" + Date.now(),
...
app.put("/api/admin/experiences/:id", requireAdmin, (req: Request, res: Response) => {
  try {
    const db = readDB();
...
app.delete("/api/admin/experiences/:id", requireAdmin, (req: Request, res: Response) => {
  try {
    const db = readDB();
...
// 7. Certificates CRUD
app.post("/api/admin/certificates", requireAdmin, (req: Request, res: Response) => {
  try {
    const db = readDB();
    const newCert = {
      id: "cert_" + Date.now(),
...
app.put("/api/admin/certificates/:id", requireAdmin, (req: Request, res: Response) => {
  try {
    const db = readDB();
...
app.delete("/api/admin/certificates/:id", requireAdmin, (req: Request, res: Response) => {
  try {
    const db = readDB();
...
app.put("/api/admin/messages/:id/reply", requireAdmin, (req: Request, res: Response) => {
  try {
    const db = readDB();
    const index = db.messages.findIndex(m => m.id === req.params.id);
    if (index === -1) {
...
app.put("/api/admin/socials", requireAdmin, (req: Request, res: Response) => {
  try {
    const db = readDB();
